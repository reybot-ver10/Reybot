...
module.exports = async function ({ globalGoat, client, api }) {
	...
	if (databaseType == "mongodb" && config.database.uriMongodb) {
		...
		const loadmongo = setInterval(() => {
			loading(P[ij++] + " Connecting to mongodb database", "MONGODB");
			ij %= P.length;
		}, 120);
		...
		try {
			...
			loading.master("Mongodb database connection successful\n", "MONGODB");
			clearInterval(loadmongo);
		}
		catch (err) {
			loading.err("Mongodb database connection failed with error: " + err.stack + "\n", "MONGODB");
			clearInterval(loadmongo);
		}
	...
	*/
	async function threadsData() {
		...
		async function saveData(threadID) {
			...
			if (databaseType == "local") {
				...
				try {
					...
					print.err("An error occurred while updating the data of the group bearing the id " + threadID + "\n" + err, "MONGODB");
				}
			}
		...
		async function setData(threadID, options, callback) {
			try {
				if (!threadID) throw new Error("threadID cannot be left blank");
				if (isNaN(threadID)) throw new Error("threadID illegal");
	...
	async function usersData() {
		...
		async function saveData(Uid) {
			...
			if (databaseType == 'local') {
				...
				await dataModels.updateOne({ type: "user" }, {
					...
					.catch(err => print.err("An error occurred while updating the user's data with id" + Uid + "\n" + err, "MONGODB"));
			}
		...
		async function createData(userID, callback) {
			try {
				if (Users[userID]) throw new Error(`User with id ${userID} already exists in database`);
		...
		async function refreshInfo(userID, callback) {
			try {
				if (!Users[userID]) throw new Error(`User with id ${userID} does not exist in database`);
		...
		async function setData(userID, options, callback) {
			try {
				if (isNaN(userID)) throw new Error("userID cannot be left blank");
				if (isNaN(userID)) throw new Error("userID illegal");
				if (typeof options != 'object') throw new Error("Options passed must be an object");
