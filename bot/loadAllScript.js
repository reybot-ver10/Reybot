	...
	for (const folderModules of folder) {
		...
		for (const file of Files) {
			try {
				...
				if (!command.start) throw new Error(`Command must not be missing function start!`);
				if (!configCommand.name) throw new Error(`Command name cannot be left blank!`);
				const commandName = configCommand.name;
				if (globalGoat[setMap].has(commandName)) throw new Error("Command name is duplicated with another Command");
				// ——————————————— CHECK SHORT NAME ———————————————— //
				if (configCommand.shortName) {
					...
					for (const aliases of shortName) {
						if (globalGoat.shortNameCommands.has(aliases)) throw new Error(`Short Name ${aliases} Duplicate with short name of command ${chalk.hex("#ff5208")(globalGoat.shortNameCommands.get(aliases))}`);
					...
					for (const i of packages) {
						try {
							...
							try {
								...
								loading(`Installed packages ${chalk.hex("#ff5208")(i)} cho ${text} ${chalk.hex("#FFFF00")(commandName)} thành công\n`, "PACKAGE");
							}
							catch (e) {
								loading.err(`Unable to install package ${chalk.hex("#ff0000")(i)} cho ${text} ${chalk.hex("#ff0000")(commandName)}\n`, "INSTALL PACKAGE FAILED");
							}
						}
					}
				}
		...
		if (commandError.length > 0) {
			print.err(`Files ${chalk.yellow(text)} error occurred during loading:`, "LOADED");
